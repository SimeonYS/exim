...
class EximPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('exim.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `exim`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM exim WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO exim (date, title, link, content)"
